...
template <class T> T Pile<T>::Remove(T obj) {
...
template <class T> void Pile<T>::Clear() {
  if (pile_.empty()) {
...
template <class T> void Pile<T>::InsertBottom(T &obj) { pile_.push_back(obj); }

template <class T> int Pile<T>::Size() const { return pile_.size(); }
